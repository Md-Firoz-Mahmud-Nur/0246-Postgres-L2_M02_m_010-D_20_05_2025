...
CREATE TABLE
  my_users (user_name VARCHAR(50), email VARCHAR(100));
...
CREATE TABLE
  deleted_users_audit (deleted_user_name VARCHAR(50), deletedAt TIMESTAMP);
...
-- Trigger
CREATE
OR REPLACE FUNCTION save_deleted_user () RETURNS TRIGGER LANGUAGE plpgsql AS
$$
    BEGIN
        INSERT INTO
          deleted_users_audit
...
CREATE
OR REPLACE TRIGGER save_deleted_user_trigger BEFORE DELETE ON my_users FOR EACH ROW EXECUTE FUNCTION save_deleted_user ();
