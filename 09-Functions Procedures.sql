...
CREATE
OR replace FUNCTION emp_count () RETURNS INT LANGUAGE SQL AS
$$
  SELECT
    COUNT(*)
  FROM
...
CREATE PROCEDURE remove_emp_var () LANGUAGE plpgsql AS
$$
  DECLARE test_var INT;

  BEGIN
    SELECT
      employee_id INTO test_var
    FROM
      employees
    WHERE
      employee_id = 26;
